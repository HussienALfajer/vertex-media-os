...
- [x] M11 Master Plan ledger, hand-off, pull request (CI evidence in the pull request)
...
- **Blocking, fixed:** T-1 (SEC-1 attribution proven for 4 of 23 mutating routes; create user and role activation, both grant paths, unproven) and T-2 (several operations and four refusal codes without evidence). Fixes in Section 8.1.
...
- **Re-check:** the tests reviewer re-ran after the fixes: T-1 to T-10 resolved (mutation-checked), no blocking finding; its minor items N-1 to N-4 were fixed. The lint rule against SYSTEM attribution is defense in depth: an indirect spelling (for example an index into a list of actor types) passes it, and the tests are the evidence (N-2).
