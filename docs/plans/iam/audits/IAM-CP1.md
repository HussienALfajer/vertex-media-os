...
**Current status:** `IAM-CP1` accepted by owner decision (Section 5.7). Re-check 1 (Section 5) returned `IAM-CP1 FIXES REQUIRED`: CP1-01 is resolved, and the fix run introduced blocking finding CP1-21, which was then fixed in a follow-up pull request without a separate fix run or re-check.  
...
**Fix runs:** `IAM-R03F` (CP1-01; merged as #9). CP1-21: fixed in a follow-up pull request by owner decision (Section 5.7).
...
| CP1-29 | Info | `IAM_R03F_SESSION_REVALIDATION_PLAN.md:138` | [SD-R2] Checklist item M8 is unticked on `main`. Commit `b7cceef` ticks it, but it was pushed after the merge and re-created `origin/iam/r03f-session-revalidation`. | `git merge-base --is-ancestor b7cceef cd82811` fails. | Ticked in the follow-up pull request; the owner may delete the remote branch |
...
- **Decision (owner, 2026-09-23):** fix CP1-21 directly in a follow-up pull request, without fix run `IAM-R03F2` and without another `/audit IAM-CP1` re-check, because the fix is small. `IAM-CP1` is accepted on that basis. This is a one-time exception to `docs/PLANNING.md` Section 9, not a change of method.
