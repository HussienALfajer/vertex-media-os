...
**Current status:** `IAM-CP1` accepted by owner decision (Section 5.7). Re-check 1 (Section 5) returned `IAM-CP1 FIXES REQUIRED`: CP1-01 is resolved, and the fix run introduced blocking finding CP1-21, which was then fixed in this record's pull request without a separate fix run or re-check.  
...
**Fix runs:** `IAM-R03F` (CP1-01; merged as #9). CP1-21: fixed in this record's pull request by owner decision (Section 5.7).
...
| CP1-29 | Info | `IAM_R03F_SESSION_REVALIDATION_PLAN.md:138` | [SD-R2] Checklist item M8 is unticked on `main`. Commit `b7cceef` ticks it, but it was pushed after the merge and re-created `origin/iam/r03f-session-revalidation`. | `git merge-base --is-ancestor b7cceef cd82811` fails. | Ticked in this record's pull request; the owner may delete the remote branch |
...
### 5.4 What the fix must deliver

- **CP1-21.** A timeout of any identity-provider call is `unavailable`, with unit tests through openid-client itself, for refresh and sign-in. Validation failures stay `rejected` (S-01).
- CP1-22 to CP1-25 were first assigned to a fix run; with the owner decision of Section 5.7 they go to IAM-MP-15.
...
### 5.7 Owner decision and the CP1-21 fix

- **Decision (owner, 2026-09-23):** fix CP1-21 directly in this record's pull request, without fix run `IAM-R03F2` and without another `/audit IAM-CP1` re-check, because the fix is small. `IAM-CP1` is accepted on that basis. This is a one-time exception to `docs/PLANNING.md` Section 9, not a change of method.
- **Fix:** `apps/api/src/auth/oidc.ts` `isUnavailable` classes openid-client's `OAUTH_TIMEOUT` and `OAUTH_ABORT` (a request that timed out or was aborted, with the `DOMException` as cause) as `unavailable` before the S-01 rule that classes every other `OAUTH_*` code as `rejected`. No other classification changed: 5xx, 429 and network failures stay `unavailable`, and validation failures and refusals stay `rejected`.
- **Evidence:**
  - Six new unit tests in `oidc.spec.ts`: refresh, code exchange and discovery, each for a timeout and for an abort, go through openid-client's own error wrapping and expect `unavailable`. Without the fix all six fail; with it, `oidc.spec.ts` passes 47/47.
  - Probe R5 re-run on the rebuilt API, against a provider that never answers: login start and refresh both give `unavailable OAUTH_TIMEOUT` after 10 s.
  - `@vertex-os/api` lint, typecheck and unit tests pass; `pnpm lint:boundaries` passes; the pull request's CI runs `pnpm verify:full` and `pnpm deps:audit`.
- **Not verified independently:** the fix was implemented and checked by the same session that audited; no fresh-context reviewer or re-check examined it.
- **Consequences:** the session service keeps a session through a Keycloak hang until its current idle deadline, without sliding it (R03F D-05, Done means 3), and a timed-out sign-in answers `IDENTITY_PROVIDER_UNAVAILABLE` again. The non-blocking findings are attached to their owner stages in the Master Plan.
