...
- Section 46.3 map (filled during the run): unique email, identity, membership, assignment and mapping; primary department; state checks; optimistic concurrency; rollback; last-admin across administrators; first activation against suspension; concurrent bootstrap; session revocation persistence.
- Section 46.6 map (filled during the run): no token; wrong token; another session's token; valid token; logout protected; safe method does not mutate; callback state mismatch.

## 9. Checklist

- [x] M0 Plan committed; Master Plan split recorded
- [ ] M1 Limiter, configuration, trust proxy; login, callback and logout limits; web `AUTH_RATE_LIMITED`
- [ ] M2 Evidence bound for denials; back-channel logout evidence (D-05)
- [ ] M3 Migration (indexes); race-safe sweep; housekeeping with retention; sign-in sweep removed; SECURITY Section 11
- [ ] M4 Back-channel logout racing a sign-in (D-08)
- [ ] M5 SA-2 and no-store; SEC-6; DC-4
- [ ] M6 Realm `delete_credential` and realm contract
