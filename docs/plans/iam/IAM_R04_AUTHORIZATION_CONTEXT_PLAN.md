...
- **D-10 Current-user capability.** The capability is `CurrentActor.require(request, reply)`, injectable in any module through the `CURRENT_ACTOR` token that the global `AuthModule` exports (its only export): any handler obtains the current actor's context without receiving the authentication runtime, its secrets or its session operations (review AB-1). Inside the authentication area it is `requireAuthorization(runtime, request, reply)`. The HTTP contract `GET /api/iam/me` (spec Section 25.2) is IAM-MP-11's deliverable ("session/current-user contracts required by the web application") and is not added here.
...
- **Public surface after D-12.** Root: `UserId`, `DepartmentId`, `PermissionCode`, `AuthorizationContext`, `hasPermission`, `SessionUser`, the sign-in, authorization and provisioning request types, `IdentityFailure`, the sign-in and authorization result types, `iamPermissionManifest` with its types, `ReferenceSyncResult` and `ReferenceSyncRefusalReason`. Composition entry: the use cases, `SignInDependencies`, `IdentityProvisioningDependencies`, `AuthorizationDependencies` and the three provisioning result types.
...
8. The context of one request is resolved at most once, by one SQL statement (the reader issues exactly one `$queryRaw`); a second request resolves it again. *(Spec Section 51; D-07, D-08.)*
