...
- **D-11 User creation (spec Section 12).** Input validation first (email normalization, display name, duplicate departments or roles, at most one primary). One transaction: lock the requested roles (D-05), refuse a missing (`role-not-found`) or INACTIVE (`role-inactive`) one, apply the grant ceiling, insert the user (`email-conflict` on an existing email, including a terminated user's), lock the departments `FOR SHARE` and refuse a missing (`department-not-found`) or INACTIVE (`department-inactive`) one, insert memberships and assignments, and append `iam.user.created` with the initial states, departments, primary and role codes. After the commit, `provisionIdentity`. An `email-conflict` makes no Keycloak call.
...
- **D-16 Bootstrap command.** `pnpm iam:bootstrap -- --email <e> --display-name <n> [--resend-invitation] [--recovery --reason <r>]`, a raw-environment bridge like `iam:sync-reference`. Exit codes: 0 complete (candidate SYNCED and invited, or nothing left to do), 2 refused, 3 incomplete (a Keycloak step failed; re-running resumes), 1 unexpected failure. It logs one JSON result line with the candidate's user ID and the outcomes, never the email, display name, reason, a token or a secret. It never creates credentials or a session.
...
- [x] M1 Plan committed
- [ ] M2 Domain rules and views; unit tests
- [ ] M3 Ports, `UserLifecycleStore` and `RoleStore` additions, adapters, removal of the unchecked repository writes
- [ ] M4 Grant ceiling in the R05 use cases; tests
- [ ] M5 Lifecycle use cases (creation, display name, restrictions, reactivation, sync, resend, revoke sessions); application tests
- [ ] M6 Bootstrap use case and `pnpm iam:bootstrap`; README
- [ ] M7 Migration (reasons, CP1-06); session binding
- [ ] M8 Integration tests; CP1-07, CP1-13, CP1-26
