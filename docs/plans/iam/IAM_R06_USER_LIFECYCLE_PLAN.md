...
| R03: session revocation capability and reasons | `SessionRevocation` port bound to the auth area's per-user revocation (the session store in the bootstrap command; `SessionService` in the HTTP runtime from IAM-MP-11, review AB-1), called after the access change commits; reasons added by migration (D-09). |
...
- **D-05 Lock order.** (0) the reference-synchronization advisory lock, bootstrap only; (1) role rows `FOR UPDATE`, several in ID order; (2) user rows `FOR UPDATE`, several in ID order; (3) department rows or permission rows `FOR SHARE`. This extends R05 D-05 across both stores. The advisory lock is taken only by bootstrap and reference synchronization, and both take it first; reference synchronization then locks the system role row before it touches any permission row (review DC-1).
...
- **D-10 Explicit revoke-sessions.** `revokeUserSessions` revokes every Vertex session of the user (`ADMINISTRATOR_REVOKED`) and then, when the user has a bound identity, ends its Keycloak sessions, so a lost device's SSO cookie cannot sign in again silently. The action always appends one record (`iam.user.sessions-revoked`, with the number of revoked sessions and the Keycloak outcome; FAILED when Keycloak failed), also when nothing was live (review SEC-4); the local revocation stands. It never changes `accessState` or `identitySyncState`.
...
- **D-16 Bootstrap command.** `pnpm iam:bootstrap -- --email <e> --display-name <n> [--resend-invitation] [--recovery --reason <r>]`, a raw-environment bridge like `iam:sync-reference`. Exit codes: 0 complete (candidate SYNCED and invited, or nothing left to do), 2 refused, 3 incomplete (a Keycloak step failed; re-running resumes), 64 usage (missing, unknown or malformed arguments), 1 unexpected failure. Its own result line carries the candidate's user ID and the outcomes, never the email, display name, reason, a token or a secret; pnpm and Nx echo the command line, arguments included, before it runs (review SEC-3). It never creates credentials or a session.
...
- **D-19 CP1-06.** Checks on `auth_session`: `idle_expires_at > created_at`, `absolute_expires_at > created_at`, non-empty token ciphertexts; on `auth_login_attempt`: non-empty `state`, `nonce`, `code_verifier`. A `BEFORE UPDATE` trigger rejects any change of `revoked_at` or `revocation_reason` on a row whose `revoked_at` is set, so a revoked session can never become valid again (spec Section 32). No application path does either today. `revoked_at >= created_at` is not checked: both values come from the application clock, and the check would make a revocation fail after a clock step, leaving sessions live (review DC-2).
...
- **Fixed in the run:** DC-1 (major; deadlock across creation, mapping replacement and reference synchronization), DC-2, DC-3, DC-5, DC-6, SEC-3 (documentation now says pnpm and Nx echo the arguments), SEC-4, AB-2 to AB-8 (Section 8.1).
- **Recorded, not changed:**
  - AB-1 (minor): the bootstrap command binds the session store directly, and `apps/api/src/iam` imports the auth area's `RevocationReason` type; the HTTP runtime must bind `AuthRuntime.sessions` (IAM-MP-11).
  - AB-9 (info): `createIamUserAdministration` has no production consumer until IAM-MP-11.
  - SEC-5 (info): a DEPRECATED permission that reference synchronization makes ACTIVE again widens roles assigned under the ceiling; it needs a code release.
  - SEC-6 (info): a throwing session revocation after a committed restriction skips reconciliation and leaves `PENDING`; access stays denied.
  - DC-4 (info): `readActorAuthority` reads facts and the system-role holding in two statements.

**Owner decision raised (SEC-2).** Reactivation has no grant ceiling: an actor holding only `iam.users.manage-access` can reactivate a SUSPENDED or DISABLED System Administrator, or any user with roles the actor lacks, and so hand back privileges the actor does not hold. Spec Sections 19 and 23.1 allow it; changing that changes the authorization model. Options: (1) apply the ceiling to reactivation (only an ACTIVE System Administrator may reactivate a System Administrator, and the actor must hold every ACTIVE permission the target's roles grant); (2) keep the specification as it is. Recommendation: option 1. Needed before IAM-MP-11 mounts `POST /api/iam/users/{userId}/reactivate`.

**Carried forward** (attached to the Master Plan stages):

- **IAM-MP-11:** the outcome mapping of Section 5.8 and names for `version-conflict`, `superseded`, `invalid`; the routes always pass the session's USER actor as attribution, never `systemAttribution` (SEC-1: the ceiling exempts every SYSTEM actor, and nothing narrows that to bootstrap); bind `revokeUserSessions` to `AuthRuntime.sessions` (AB-1); the owner decision SEC-2; `UserView` as the user DTO without the identity mapping.
- **IAM-MP-15:** SEC-5, SEC-6 and DC-4.
- **Open item (no stage):** any later administrative recovery action (spec Section 54) must not be runnable through an unexpired invitation link (R02 S-06, S-07; D-17).