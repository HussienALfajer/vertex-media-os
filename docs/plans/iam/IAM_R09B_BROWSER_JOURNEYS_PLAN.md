...
- **D-03 A black-box stack started in global setup.** The global setup starts PostgreSQL, Mailpit and Keycloak through the Docker CLI (Section 9.1), applies the migrations with the Prisma CLI, runs the built operator commands (`iam-sync-reference`, `iam-bootstrap`), then starts the built API server entry (`apps/api/dist/main.js`) and the production web preview as child processes with an explicit environment: the API and the commands read no `.env`, and the values the stack sets win over the `.env` fallbacks of the Prisma CLI and the Vite preview. Its teardown stops everything. Images and the realm come from `infra/compose.yaml` and `infra/keycloak/`, never copied. Global setup, not `webServer`, because container ports and generated secrets exist only after the containers start. The e2e project imports no workspace code (Nx tag `type:e2e`), so it cannot share `apps/api/test-support`; the Section 15 open item on a shared test-support package is not affected. The harness lives in `apps/web-e2e/test-support/iam/`, the specs in `apps/web-e2e/iam/`, outside the smoke configuration's `testDir` (review A-1).
...
- **D-08 Back-channel path (CP1-17).** The realm's back-channel URL reaches the loopback API the way the Compose realm does on Docker Desktop hosts (`host.docker.internal`). On Linux that name does not reach a loopback port, so Keycloak shares the host network there and calls `127.0.0.1` (Section 9.1). A local run on Windows therefore proves the local Compose path; CI proves the Linux path.
- **D-09 Login-attempt volume (R09 review S-3).** No process-wide sign-in ceiling: one flood could then deny sign-in to every staff member, trading bounded storage growth for an outage. Per-address limits and housekeeping stay. The residual (many addresses under the limit) is recorded for the production deployment design, which chooses between per-address limits at the reverse proxy with an alert on `auth_login_attempt` growth, and an explicitly accepted trade-off; a global ceiling at the proxy would reintroduce the lockout this decision avoids.
...
- **Hand-over to workers.** The setup publishes the Keycloak issuer, Mailpit URL, database access and the administrator's storage-state path through `process.env`, which Playwright passes to its workers. The stack's configured secrets (client secrets, the Keycloak administrator, the database password, the token key) never enter `test-output`. The trace and screenshot of a failed test can hold that test's throwaway session values, TOTP seed and the test password; they die with the stack at teardown, before CI uploads `test-output`.
...
- [x] M2 Session journeys: J-01, J-02, J-07, J-09, J-10, J-11
- [x] M3 Administration journeys: J-03 to J-06, J-08, J-12, J-13
- [x] M4 Closeout: Definition of Done map, sweep, dependency audit, documentation
- [x] M5 `pnpm verify`, `pnpm test:e2e`, mutation probes
- [x] M6 In-run review; fixes
- [x] M7 Master Plan ledger and hand-off; pull request; CI

### 9.1 Deviations and discoveries

- **Docker CLI instead of Testcontainers (D-03).** `apps/web-e2e` carries the Nx tag `scope:web`, whose boundary rule bans `testcontainers` for every file of the project. The harness drives Docker through its CLI, as the visual-browser setup does, with fixed container names so a run removes what an interrupted run left behind. On Linux, Keycloak runs on the host network (`KC_HTTP_HOST=127.0.0.1` and free ports) so it can reach the loopback API; elsewhere it runs on a stack network and calls `host.docker.internal`.
- **Harness outside `src/` (D-03).** The repository keeps test harnesses outside `src/` (`apps/api/test-support`), where the rule against raw environment reads and SQL strings does not apply; the IAM harness follows it.
- **Playwright's HTTP client and `Secure` cookies.** Playwright's own request client keeps no `Secure` cookie for a plain-HTTP URL, and `context.cookies(url)` leaves them out for one. The administrator's API client sends its cookie as a header, the journeys read cookies unfiltered, and in-page checks use the page's own `fetch`.
- **A refused request clears the cookie.** The API clears the session cookie with every refusal, so a probe from the page consumes the signed-out reason. J-09 therefore waits for the revocation in PostgreSQL before the page reloads.
- **Keycloak's session answers a second sign-in at once.** While the Keycloak session lives, sign-in returns without a form (J-07 expiry, J-10 rotation); the helper waits for either the form or the signed-in shell.
- **Firefox and JSON responses.** Firefox's JSON viewer never finishes loading a JSON navigation, so the script-visible check reads from the app page.
- **Mutation probes.** A JWT answered to `/api/auth/session` fails the D-12 check; a handled secret in the API log fails the run in teardown (D-13); J-05 without its save step fails on the holder's `200`.
- **Temporary-code sweep (D-18).** No focused or skipped test, `debugger`, stray console output or TODO/FIXME/HACK marker in `apps`, `domains`, `packages`, `scripts` or `infra`. The only matches for insecure or bypass wording are deliberate: the OIDC client accepts an HTTP issuer outside production (`auth-config.ts`, `NODE_ENV !== 'production'`), OpenAPI generation uses a never-contacted placeholder issuer, and the CSP leaves `upgrade-insecure-requests` to HSTS once deployed behind TLS.
- **Dependency audit (D-17).** `pnpm deps:audit` passes with the four reviewed exceptions of `pnpm-workspace.yaml` (one moderate, three high; each pinned by Nx or the Prisma CLI outside the API runtime path).
- **IAM specs outside `src/` (review A-1).** The Nx Playwright plugin infers an `e2e-ci` target per spec under the smoke configuration's `testDir` and ignores `testIgnore`, so the IAM specs moved to `apps/web-e2e/iam/`.
- **Stronger evidence after review.** The D-12 check reads every response header (`Set-Cookie` included), the values of the cookies the app receives, and web storage in each open page, where only the UI preference may live; a real `Set-Cookie` with a JWT and a stray storage entry each fail it (probed). The D-13 scan also rejects `code=`/`state=` parameters and `__Host-vertex` text, includes every journey's session values and TOTP seeds, waits for the log file to close, and fails without a sign-in callback in the log (positive control). Clean logs are copied to `test-output/iam-logs` for CI's failure artifact. A process that exits during setup ends the wait at once. The temporary directory has a fixed name, so an interrupted run's secrets are removed by the next run. Secrets reach containers through environment files only.
## 10. Hand-off

**In-run review.** Three fresh-context reviewers checked `main...HEAD`: security; tests and verification; architecture and boundaries (D-01). The implementer checked the evidence of each finding before acting on it.

- **Blocking, fixed:** S-01 (the production items of D-06, D-07, D-09 and the D-11 trigger were not yet in Master Plan Section 15, which the Definition of Done map cites). They are recorded there now.
- **Fixed in the run:** A-1 (specs moved out of the smoke `testDir`), A-3 and A-4 (README, plan and map wording), A-5 (stale comments in the smoke configuration, `infra/` and the project description), S-02, S-03, S-05, S-09, T-1, T-2, T-6 (Section 9.1), S-06 (J-02 asserts `AUTH_SESSION_INVALID` and the `LOGOUT` revocation), S-07 (D-09 residual reworded), S-10 (J-10 asserts the login cookie's attributes; README wording on WebKit), S-11 and T-3 (J-13 asserts the System Administrator assignment is kept), T-4 (bootstrap refusals assert their logged reason), T-5 (the two Linux-dependent map rows say "when CI is green"), and the README probe count.
- **Recorded, not changed:** S-04 (a failed test's trace can hold that test's throwaway session values and TOTP seed; the stack dies at teardown before the upload, plan Section 5), S-08 (normal-mode refusal reason now asserted, T-4), A-6 (the suite reads and writes three tables through `psql` and runs other projects' installed binaries by path; authorized by D-10, D-14 and D-15, test-only), T-7 (one IAM stack per host: fixed ports and container names).
- **Carried forward:** A-2 (pre-existing: `@nx/enforce-module-boundaries` does not lint `.mts` files) to `IAM-FINAL`.

**Carried forward** (attached to the Master Plan):

- **`IAM-FINAL` (Section 17):** the Definition of Done map as its input; reactivated permissions (IAM-R09 D-12); `.mts` module boundaries (review A-2).
- **Production deployment design (Section 15):** WebKit/Safari over the HTTPS origin (D-06); Keycloak on the web app's registrable domain (D-07); sign-in volume (D-09).
- **Open item (Section 15):** the permission catalog page bound (D-11).
- **First CI run:** the Linux path of the stack (Keycloak on the host network, WebKit on Linux, back-channel logout to `127.0.0.1`) runs for the first time in this pull request's CI.