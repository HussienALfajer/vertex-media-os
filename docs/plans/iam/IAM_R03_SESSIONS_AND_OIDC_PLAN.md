...
- **D-04 Expiry.** `idle_expires_at = min(now + idle, absolute_expires_at)`, `absolute_expires_at = created_at + absolute`. A session is valid when not revoked and both deadlines are in the future. Validation slides the idle deadline only when at least 60 s have passed since `last_seen_at`, with a conditional update that never revives an expired or revoked row. Time comes from an injected clock (tests) that defaults to the system clock. *Revised by IAM-R03F (D-02, D-04):* the single conditional touch became a claim (a conditional update of `last_seen_at` only, so one request per interval wins) followed, after Keycloak refreshed its session, by a conditional apply of the slid deadline and the rotated tokens. The apply also requires that the row still holds the claim and the refresh token that was refreshed.
- **D-05 Limits.** Defaults: idle 1800 s (allowed 300–3600), absolute 36000 s (3600–86400, at least idle), login attempt 600 s (60–1800). The defaults equal the realm's SSO idle and maximum, so SECURITY Section 11 holds without a realm change. `.env.example` states that lowering them below the realm values requires lowering the realm. *Revised by IAM-R03F (D-02):* equal limits did not keep the two sessions together, because the API never contacted Keycloak after the callback, so the Keycloak session idled out while the Vertex session kept sliding (`IAM-CP1` CP1-01). The idle deadline now slides only after Keycloak refreshed its session, so the effective idle limit is the smaller of the two.
...
7. Logout revokes the session, clears the cookie and returns an end-session URL that ends the Keycloak session; a back-channel logout from Keycloak revokes the matching Vertex sessions, and an invalid logout token revokes nothing. *(MP-06 exit 7.)* *Corrected by IAM-R03F (`IAM-CP1` CP1-16):* since the S-01 fix (D-17) the API ends the Keycloak session itself and returns the post-logout URI; it returns the token-free end-session URL only as a fallback.
