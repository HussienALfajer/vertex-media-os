...
- [x] M9 In-run review and fixes
- [x] M10 Master Plan ledger, hand-off, pull request, CI

### 9.1 Deviations and discoveries

- **D-12 as built (review A-1).** Only the code input carries `maxLength` (codes are ASCII). IAM measures names and descriptions in code points after trimming, which an input's UTF-16 `maxLength` cannot express, so those bounds are left to the API's `400`, which marks the field.
- **Refused codes reload the catalog too.** After `IAM_UNKNOWN_PERMISSION` or `IAM_PERMISSION_NOT_ASSIGNABLE` the editor re-reads the role and the catalog, and a choice the reloaded catalog no longer lists as ACTIVE is not requested again.
- **Shared list page.** Departments, roles and the permission catalog share one list component (`organization/state-list-page.tsx`) and one address-state helper (`list-search.ts`); the user directory keeps its own, as built in R08B.
- **Test helpers.** `organization.spec.tsx` has its own small fake API instead of sharing `users.spec.tsx`'s: a shared helper outside `*.spec.*` would enter the application build under the current `tsconfig` split.
- **Generated route tree.** `apps/web/src/routeTree.gen.ts` is regenerated by the TanStack Router plugin; not edited by hand (the architecture reviewer regenerated it byte-identically).

## 10. Hand-off

**In-run review.** Two fresh-context reviewers checked `main...HEAD`: architecture and boundaries; tests and verification (D-01). The implementer checked the evidence of each finding before acting on it; each held.

- **Blocking, fixed:** T-1 (an unconfirmed name/description edit said the record was reloaded but did not re-read it; the edit dialog now reloads on every reload outcome, tested), T-2 (no tests for `IAM_SYSTEM_ROLE_PROTECTED`, `IAM_ROLE_CODE_CONFLICT`, `IAM_UNKNOWN_PERMISSION`; added at unit and component level), T-3 (activation confirmation's role name, code and reach untested; added).
- **Fixed in the run:** T-4 to T-10 (privileged warning absent for other sensitivities, no actions without the manage permissions, role kind in the list by `isSystem`, role creation and edit wiring, own-access re-read after a role change, activation intent, editor dirty guard and truncation note, English LTR scans of role detail, the editor review and the department deactivation); the architecture reviewer's out-of-concern note (the catalog was not re-read after a refused code, although the message said so); A-1 (see 9.1).
- **Recorded, not changed:** A-2 (`SEARCH_MAX` and page parsing exist in the user directory and in the shared list helpers; moving the directory onto them would reach into user administration beyond this run's scope), A-3 (shared helpers such as `ReasonField` and `PickerNote` still live under `users/`), T-11 (evidence of `pnpm verify` and `pnpm test:e2e` is in the pull request report).

**Carried forward** (attached to the Master Plan stages):

- **IAM-MP-15:** signed-in privilege administration through the real API and Keycloak, including the effect of a mapping change on a holder's next request (D-14); the one-page catalog bound in the editor (D-08).
