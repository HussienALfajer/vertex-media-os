...
Pagination, filtering, sorting, and search conventions MUST be consistent across endpoints. The convention, first defined by IAM (`docs/plans/iam/IAM_R07_HTTP_ADMINISTRATION_PLAN.md` D-03): collections take `page` (1–10 000) and `pageSize` (1–100, default 25) and answer `{ items, page, pageSize, total }` in a fixed, total order; `search` is a literal, case-insensitive substring (1–100 characters after trimming); filters are named query parameters; a collection route refuses unknown query parameters.
